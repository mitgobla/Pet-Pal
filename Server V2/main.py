...
EMULATE = False
EMULATE_TEMP = b'Hot'
EMULATE_BOWL_STATUS = "2"
EMULATE_BOWL_DEPTH = b'Full'
EMULATE_MODE = "Temp"

try:
    import pigpio #Used for the control of LEDs
except ImportError:
    print("You are not running on a Raspberry Pi.\nUsing Emulator Mode")
    import tkinter #Used to emulate the hub itself
    EMULATE = True
...
class Hub():
    ...
    def add_temperature_module(self):
        ...
        if EMULATE:
            self.temp_module.set_temperature(EMULATE_TEMP)
        else:
    ...
    def run_server(self):
        ...
        if EMULATE:
            self.bowl_module.set_status(EMULATE_BOWL_STATUS)
            self.bowl_module.set_depth(EMULATE_BOWL_DEPTH)
            self.mode = EMULATE_MODE
            Thread(target=self.run_visuals()).start()
    ...
    def run_visuals(self):
        ...
        print("Running visuals ", self.server_running)
        if EMULATE:
            window = tkinter.Tk()
            colour_label = tkinter.Label(window, text="LED")
            colour_label.pack()
            window.update()
            window.update_idletasks()
            def update_gui_led(r, g, b, w):
                """Change the TKINTER Label colour"""
                tk_rgb = "#%02x%02x%02x" % (r, g, b)
                colour_label.config(fg=tk_rgb)
                w.update()
                w.update_idletasks()
        else:
            window = None
        ...
        while self.server_running:
            ...
            while self.mode == "Bowl":
                ...
                if status == "1":
                    ...
                    self.led_module.blue(0)
                    update_gui_led(255, 0, 0, window)
                elif status == "2":
                    print("LED PURPLE")
                    self.led_module.red(255)
                    self.led_module.green(0)
                    self.led_module.blue(255)
                    update_gui_led(255, 0, 255, window)
                elif status == "3":
                    print("LED CYAN")
                    self.led_module.red(0)
                    self.led_module.green(255)
                    self.led_module.blue(255)
                    update_gui_led(0, 255, 255, window)
                elif status == "4":
                    ...
                    if depth == b'Full':
                        ...
                        self.led_module.green(255)
                        self.led_module.blue(0)
                        update_gui_led(0, 255, 0, window)
                    elif depth == b'Low':
                        self.led_module.red(255)
                        self.led_module.green(255)
                        self.led_module.blue(0)
                        update_gui_led(255, 255, 0, window)
                    else:
                        self.led_module.red(255)
                        self.led_module.green(0)
                        self.led_module.blue(0)
